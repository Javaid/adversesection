...
function Identifiers({ provider, refreshProvider }) {
  ...
  // Sync identifiers data from provider
  useEffect(() => {
    const data = provider?.identifiers;
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await api.post(`/v1/providerss/${provider.id}/identifiers`, payload);
  ...
  return (
    <div className="bg-white border border-[#d8e4ef] rounded-xl p-6 mb-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold text-[#2e4358]">

...
function HealthInfoExchange({ provider }) {
  ...
  // Sync health info data from provider
  useEffect(() => {
    const data = provider?.healthInfo;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/v1/providerss/health_info/${id}`);
      fetchHealthInfo();
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      await api.post(`/v1/providerss/${provider.id}/healthinfo`, formData);
      fetchHealthInfo();

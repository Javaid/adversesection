...
function Taxonomy({ provider }) {
    ...
    const handleSave = async () => {
        try {
            if (editingId) {
                await api.put(`/v1/providerss/${provider.id}/taxonomy/${editingId}`, formData);
            } else {
                await api.post(`/v1/providerss/${provider.id}/taxonomy`, formData);
            }

  ...
  return (
    <header className="flex flex-col md:flex-row items-center justify-between p-4 bg-white shadow-sm border-b border-[#d8e4ef] min-h-[64px]">
      ...
      <div className="relative" ref={menuRef}>
        ...
        {showMenu && (
          <ul className="absolute right-0 mt-2 w-44 bg-white border border-[#d8e4ef] rounded-md shadow-lg z-50 text-sm text-[#2e4358]">
            {user?.role === "superAdmin" && (
              <>
                <li className="px-4 py-2 hover:bg-[#f3f8fc] cursor-pointer" onClick={() => { navigate("/dashboard"); setShowMenu(false); }}>Dashboard</li>
                <li className="px-4 py-2 hover:bg-[#f3f8fc] cursor-pointer" onClick={() => { navigate("/companies"); setShowMenu(false); }}>Companies</li>
                <li className="px-4 py-2 hover:bg-[#f3f8fc] cursor-pointer" onClick={() => { navigate("/settings"); setShowMenu(false); }}>Settings</li>
                <li className="px-4 py-2 hover:bg-[#f3f8fc] cursor-pointer" onClick={() => { navigate("/providers"); setShowMenu(false); }}>Providers</li>
                <li className="px-4 py-2 hover:bg-[#f3f8fc] cursor-pointer border-t border-[#d8e4ef]" onClick={handleLogout}>Logout</li>
              </>
            )}
            {user?.role === "admin" && (
              <>
            ...
            {(!user || (user.role !== "superAdmin" && user.role !== "admin")) && (
              <li className="px-4 py-2 hover:bg-[#f3f8fc] cursor-pointer" onClick={handleLogout}>Logout</li>
            )}
          </ul>
        )}
      </div>
